...
describe('Badge', () => {
    ...
    it('derives the subtle class from variant + contrast="low"', () => {
        ...
        const el = screen.getByText('blue subtle').closest('[class*="badge"]');
    ...
    it('derives subtle for every color variant with contrast="low"', () => {
        ...
        colors.forEach((color) => {
            ...
            const el = screen.getByText(`${color} subtle`).closest('[class*="badge"]');
    ...
    it('does NOT apply the subtle class without contrast', () => {
        ...
        const el = screen.getByText('solid blue').closest('[class*="badge"]');
        expect(el?.className).toContain('blue');
    ...
    it('applies size class — sm', () => {
        ...
        const el = screen.getByText('small').closest('[class*="badge"]');
        expect(el?.className).toContain('sm');
    });

    it('applies size class — md', () => {
        render(<Badge size="md">medium</Badge>);
        const el = screen.getByText('medium').closest('[class*="badge"]');
        expect(el?.className).toContain('md');
    });

    it('applies size class — lg', () => {
        render(<Badge size="lg">large</Badge>);
        const el = screen.getByText('large').closest('[class*="badge"]');
    ...
    it('renders as div by default', () => {
        ...
        const el = screen.getByText('span badge').closest('[class*="badge"]');
        expect(el?.tagName).toBe('DIV');
    });

    it('renders as anchor when href is provided', () => {
        render(<Badge href="https://example.com">link badge</Badge>);
        const el = screen.getByText('link badge').closest('[class*="badge"]');
    ...
    it('does NOT carry component markers (production Badge is marker-free)', () => {
        // Terukur 31 Agu 2026: 79 badge produksi tanpa data-slot/data-version/
        // penanda apa pun, murni kelas. Penanda kita dicabut agar identik.
        render(<Badge>attr test</Badge>);
        const el = screen.getByText('attr test').closest('[class*="badge"]');
        expect(el).not.toHaveAttribute('data-oxobz-badge');
        expect(el).not.toHaveAttribute('data-version');
    });

    it('applies custom className', () => {
        render(<Badge className="my-custom-class">custom</Badge>);
        const el = screen.getByText('custom').closest('[class*="badge"]');
