...
function getRoot(container: HTMLElement) {
    return container.querySelector('[data-testid="geistcn/status-dot"]');
}

/** Selects the colored dot (first span child of the root). */
function getDot(container: HTMLElement) {
    return container.querySelector('[data-testid="geistcn/status-dot"] > span');
}
...
describe('StatusDot', () => {
    ...
    it('renders a root span with the geistcn test id and no marker attributes', () => {
        ...
        expect(root).not.toHaveAttribute('data-oxobz-status-dot');
    ...
    it.each(stateInfo)('%s sets aria-label "%s"', (state, name) => {
        const { container } = render(<StatusDot state={state} />);
        expect(getRoot(container)).toHaveAttribute('aria-label', name);
    });

    it.each(stateInfo)('%s composes the default title "%s"', (state, _name, title) => {
        const { container } = render(<StatusDot state={state} />);
        expect(getRoot(container)).toHaveAttribute('title', title);
    });

    it.each(stateInfo)('%s applies the expected dot color class', (state, _name, _title, color) => {
        const { container } = render(<StatusDot state={state} />);
        const dot = getDot(container);
        expect(dot?.className).toContain('status');
    ...
    it('composes the title from a custom titlePrefix', () => {
        ...
        expect(getRoot(container)).toHaveAttribute('title', 'vercel-site production is building.');
    });
    ...
    it('renders no visible label by default', () => {
        ...
        expect(container.querySelector('span[class*="statusLabel"]')).not.toBeInTheDocument();
    });
    ...
    it('appends a custom className after the module class', () => {
        const { container } = render(<StatusDot className="custom-dot" state="READY" />);
        const root = getRoot(container);
        expect(root?.className).toContain('wrapper');
    ...
    it('forwards ref to the root span', () => {
        ...
        expect(ref.current).toHaveAttribute('data-testid', 'geistcn/status-dot');
    });
    ...
    it('forwards extra HTML attributes (id, aria-hidden, style)', () => {
        const { container } = render(
            <StatusDot aria-hidden="true" id="dot-1" state="READY" style={{ marginTop: '4px' }} />,
        );
        const root = getRoot(container);
